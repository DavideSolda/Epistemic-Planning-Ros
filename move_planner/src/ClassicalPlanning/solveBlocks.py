...
def getActions():
    os.system("clingo lp/clingoIstance.lp lp/add4Blocks.lp | tail -10 | head -1 > output/actions.txt")
    actions = open("output/actions.txt", "r")
...
def execute_actions(move_group, pub, display_trajectory_publisher, actions):
    ...
    for action in actions:
        print("init action"+str(action)+" pose:")
...
def let_down(move_group, display_trajectory_publisher, action):
    ...
    pose_goal.position.z = z_down#(int(action[2])-1)*height_block+z_down
...
def move_robot(move_group, display_trajectory_publisher, action):
    ...
    wpose.position.x = x_position[int(action[3])-1]
