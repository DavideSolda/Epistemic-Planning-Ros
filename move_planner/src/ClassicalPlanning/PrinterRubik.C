...
void PrintAsp::print(Visitable *v){ 
  ...
  //f << "not_ok(T) :- time(T), finally(F), not holds(F,T).\n";
  f << "goal(T) :- time(T), not not_ok(T).\n";
  f << ":- not goal(l).\n";
  f << "opposto(F,neg(F)):- fluent(F).\n";
  f << "opposto(neg(F),F):- fluent(F).\n";
  //f << "not_executable(A,T):- time(T), exec(A,F), not holds(F,T).\n";
  //f << "executable(A,T):- action(A), time(T), T<l, not not_executable(A,T).\n";
  //f << "holds(F,T+1):- T<l, executable(A,T), occurs(A,T), causes(A,F).\n";
  f << "holds(F,T+1):- T<l, occurs(A,T), causes(A,F,T).\n"; // NEW ENTRY!!!!
  //f << "holds(F,T+1):- T<l, executable(A,T), occurs(A,T), causes(A,F,T).\n"; // NEW ENTRY!!!!
  f << "holds(F,T+1):-opposto(F,G), T<l, holds(F,T), not holds(G,T+1).\n";
  f << "occurs(A,T):- action(A), time(T), not goal(T), not not_occurs(A,T).\n";
  f << "not_occurs(A,T):- action(A), action(B), time(T), occurs(B,T), A!=B.\n";
  //f << ":- action(A), time(T), occurs(A,T), not executable(A,T).\n";
  f << "action(noop).\n";
  f << "holds(F,1):-initially(F).\n";
  //f << "holds(has_col(C,P,C1)):- holds(has_col(C,P,C1),l), color(C), color(C1), position(P).\n";
  //f << "holds(has_pos(S,C)):- holds(has_pos(S,C),l), color(C), surface(S).\n";
  //f << "holds(neg(has_col(C,P,C1))):- holds(neg(has_col(C,P,C1)),l), color(C), color(C1), position(P).\n";

  //f << "#maximize { T:occurs(noop,T) }.\n";
  f << "#show occurs/2.\n";
  f << "#show holds/2.\n";
  //f << "#show crossOk/1.\n";
  //f << "occurs(rotateDoubleFore(180),1).\n";
  //f << "occurs(rotateBack(180),2).\n";
  //f << "occurs(rotateDoubleFore(180),1).\n";
  //f << "occurs(rotateXY(180),1).\n";
  //f << "occurs(rotateXY(90),2)\n";
  //f << "occurs(rotateLeft(90),1).\n";
  //f << "occurs(rotateBack(90),1).\n";
  //f << "occurs(rotateBack(180),2).\n";
  //f << "occurs(rotateBack(270),3).\n";
...
void PrintAsp::visitExec(Exec* p)
{
  ...
  for(Literal* lit : *listLit){
    lit->accept(this);
    std::cout<<"exec("+actionString + ", " + literalString+")"; 
    f<<"exec("+actionString + ", " + literalString+")";
    literalString="";
...
void PrintAsp::visitCaus(Caus* p)
{
  ...
  for(Literal* lit : *listLit){
    lit->accept(this);
    f<<"causes("+actionString + ", " + literalString+")";
    std::cout<<"causes("+actionString + ", " + literalString+")";
    literalString="";
    if(varsAction.size()+varsLiteral.size()>0){
      std::string str;
      varsLiteral.insert(varsLiteral.end(), varsAction.begin(), varsAction.end());
...
void PrintAsp::visitPreds(Preds* p)
{
  ...
  f<<"fluent("+predString+")"+vars2print+".\n";
...
void PrintAsp::visitAct(Act* p){actionString=p->ident_; f<<"action("+actionString+").\n"; varsAction.clear();}
